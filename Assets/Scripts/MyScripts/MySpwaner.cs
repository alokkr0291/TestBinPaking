...
{ 
  ...
  void Init()
  {
    ...
    for (int i = 0; i < packedboxes.BestResult[0].Count; i++)
    {
      ...
      {
      // Debug.Log($" tag --> {box.Tag} --> {box.X } -{box.Y}- {box.Z}");
       // Debug.Log($" l,b,f--> {box.Width } -{box.Height}- {box.Depth}");
  ...
  private static void AssignInfo(GameObject obj, Cuboid box, string tag)
  {
    obj.transform.position = new Vector3((float) (box.X), (float) box.Y, (float) box.Z);
