...
namespace MobackPacker
{
    ...
    public class BinPacker : IBinPacker
    {
        ...
        public BinPackResult Pack(BinPackParameter parameter)
        {
            ...
            {
                Debug.Log($"pendingCuboids.count --> {pendingCuboids.Count}");
                // pack a single bin
        ...
        public static void Verify(BinPackParameter parameter, string algorithmName, IList<Cuboid> cuboids)
        {
            ...
            for (int a = 0; a < cuboids.Count; ++a)
            {
                ...
                {
                    Debug.LogError($"verify cuboid failed: negative position, algorithm: {algorithmName}, cuboid: {cuboid}");
                ...
                {
                    Debug.LogError($"verify cuboid failed: out of bin, algorithm: {algorithmName}, cuboid: {cuboid}");
                    throw new ArithmeticException(
                ...
                for (int b = a + 1; b < cuboids.Count; ++b)
                {
                    ...
                    {
                        Debug.LogError($"verify cuboid failed: cuboid intersects others, algorithm: {algorithmName}, cuboid a: {cuboid}, cuboid b: {otherCuboid}");
                        throw new ArithmeticException(
            ...
            {
                Debug.LogError($"verify cuboid failed: cuboids overweight, algorithm: {algorithmName}");
                throw new ArithmeticException(
        ...
        {
            Debug.Log($"BinPackAlgorithmFactory");
            return new BinPackAlgorithmFactory[]
            {
                ...
                parameter => new BinPackGuillotineAlgorithm(
                    parameter,
                    FreeCuboidChoiceHeuristic.CuboidMinWidth,
